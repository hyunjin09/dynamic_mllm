...
RUN_ID=${RUN_ID:-reproduced_sw31_core_vqa}
OUT_ROOT="$BUNDLE/results/core_vqa_regeneration"
...
DATA="$BUNDLE/data/heldout_lmms_recommended_v1"
ROUTER="$BUNDLE/checkpoints/sw31/router_epoch_001.pt"
BASELINE="$BUNDLE/baseline/core_vqa_all_on_generation_rows.jsonl"
...
for ((gpu=0; gpu<NUM_GPUS; gpu++)); do
  (
    ...
    cmd=(
      ...
      --process-name "repro-core-s${gpu}"
      "${baseline_args[@]}"
    )
    CUDA_VISIBLE_DEVICES="$gpu" "${cmd[@]}" 2>&1 |
      tee "$BUNDLE/results/logs/core_vqa_shard_${gpu}.log"
  ) &
  pids+=("$!")
done
for pid in "${pids[@]}"; do
