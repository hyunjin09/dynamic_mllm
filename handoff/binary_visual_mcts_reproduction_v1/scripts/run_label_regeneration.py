...
def record_complete(path: Path, *, uid: str, contract_hash: str) -> bool:
    if not path.is_file():
        return False
    try:
    ...
    return (
        ...
        and record.get("runtime", {}).get("contract_sha256") == contract_hash
        and isinstance(record.get("candidate_executions"), list)
        and record.get("mcts", {}).get("completed_simulations")
        == record.get("mcts", {}).get("requested_simulations")
    )
...
def find_completed_record(
    ...
    *,
    ...
    for path in existing_records.get(filename, ()):
        if record_complete(path, uid=uid, contract_hash=contract_hash):
            return path
    return None
...
def run_mcts(args, rows, *, rank: int, world_size: int) -> None:
    ...
    for local_index, sample in enumerate(shard_rows):
        ...
        try:
            ...
            if not root_correct and not any(row["result_correct"] for row in search.evaluations.values()):
...
def main() -> None:
    ...
    parser.add_argument(
        "--dataset-version",
        required=True,
        help="Immutable human-readable identifier for the input dataset/manifest.",
    )
    parser.add_argument(
        "--phase-name",
        default="binary_visual_mask_graph_mcts_regenerated_v1",
    )
