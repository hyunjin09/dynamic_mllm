...
def wemath2pro_mathruler_accuracy(prediction: str, answer: str) -> float:
    """Apply the exact MathRuler reward contract used to create MCTS labels."""
    try:
        from mathruler.grader import grade_answer
    except ImportError as exc:  # pragma: no cover - execution environment failure
        raise RuntimeError(
...
def score_prediction(
    ...
    if metric in {"wemath20_mathruler_accuracy", "wemath2pro_mathruler_accuracy"}:
