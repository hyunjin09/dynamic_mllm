...
from dvr_qwen.masks import make_text_causal_mask
from dvr_qwen.split_scatter import BinaryDVRCIndexCache, BinaryDVRCInputs, scatter_to_full, split_from_full
...
def _module_device(module: torch.nn.Module) -> torch.device:
    ...
    if device.type != "meta":
        return device
    # Accelerate CPU offload leaves parameters on the meta device between
    # calls. Its hook records the real device on which inputs and parameters
    # must meet for the layer forward.
    hook = getattr(module, "_hf_hook", None)
    execution_device = getattr(hook, "execution_device", None)
    if execution_device is None:
        raise RuntimeError(f"{type(module).__name__} is on meta without an Accelerate execution device")
