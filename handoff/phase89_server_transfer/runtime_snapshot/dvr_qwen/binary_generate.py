...
class BinaryDVRCPrefillState:
    ...
    forced_visual_on_prefix_layers: int = 0
    prefix_gate_features: dict[str, torch.Tensor] | None = None
    prefix_admission_used_sparse: bool | None = None
...
class BinaryDVRCGenerationOutput:
    ...
    generated_token_logprobs: torch.Tensor | None = None
    generated_token_entropies: torch.Tensor | None = None
    generated_token_top1_top2_margins: torch.Tensor | None = None
...
def selected_token_distribution_stats(
    logits: torch.Tensor, token_ids: torch.Tensor
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Return selected-token logprob, entropy, and top-1/top-2 logit margin."""

    scores = torch.as_tensor(logits, dtype=torch.float32)
    tokens = torch.as_tensor(token_ids, dtype=torch.long, device=scores.device).view(-1)
    if scores.ndim != 2 or scores.shape[0] != tokens.numel() or scores.shape[1] < 2:
        raise ValueError("logits/token IDs must have aligned [batch, vocabulary] shapes")
    log_probs = torch.log_softmax(scores, dim=-1)
    selected = log_probs.gather(1, tokens[:, None]).squeeze(1)
    entropy = -(log_probs.exp() * log_probs).sum(dim=-1)
    top2 = torch.topk(scores, 2, dim=-1).values
    margin = top2[:, 0] - top2[:, 1]
    return selected, entropy, margin
...
def validate_prefix_layers(prefix_layers: int, num_layers: int) -> int:
    if isinstance(prefix_layers, bool) or not isinstance(prefix_layers, int):
        raise TypeError("forced_visual_on_prefix_layers must be an integer")
    if prefix_layers < 0 or prefix_layers > num_layers:
        raise ValueError(
            f"forced_visual_on_prefix_layers must be in [0, {num_layers}], got {prefix_layers}"
        )
    return prefix_layers


def prefix_forced_route_decision(
    layer_idx: int,
    prefix_layers: int,
    *,
    predicted_visual_on: bool,
) -> bool:
    """Apply a shared dense prefix before returning control to the online policy."""

    return True if layer_idx < prefix_layers else bool(predicted_visual_on)


def prefix_admitted_route_decision(
    layer_idx: int,
    prefix_layers: int,
    *,
    ...
    """Apply the shared-prefix and conservative all-on fallback contract."""

    if layer_idx < prefix_layers:
        return True
    if use_sparse is False:
        return True
    return bool(predicted_visual_on)
...
def binary_dvrc_prefill(
    ...
    forced_visual_on_prefix_layers: int = 0,
    capture_prefix_gate_features: bool = False,
    prefix_admission_callback: Callable[[dict[str, torch.Tensor]], bool] | None = None,
    ...
    with torch.no_grad():
        ...
        prefix_layers = validate_prefix_layers(forced_visual_on_prefix_layers, num_layers)
        if prefix_layers and route_mode != _ROUTER_ONLINE_MODE:
            raise ValueError(
                "a forced visual-on prefix is currently supported only with router_online mode"
            )
        if capture_prefix_gate_features and route_mode != _ROUTER_ONLINE_MODE:
            raise ValueError("prefix gate features currently require router_online mode")
        if prefix_admission_callback is not None:
            if route_mode != _ROUTER_ONLINE_MODE:
                raise ValueError("prefix admission currently requires router_online mode")
            if not capture_prefix_gate_features or not 0 < prefix_layers < num_layers:
                raise ValueError(
                    "prefix admission requires feature capture and 0 < prefix layers < model depth"
                )
        ...
        for layer_idx, layer in enumerate(text_model.layers):
            ...
            if route_mode in _ONLINE_ROUTE_MODES:
                ...
                if route_mode == _ROUTER_ONLINE_MODE:
                    predicted_visual_on = bool(
                        (layer_logit.detach()[0] > float(router_threshold)).item()
                    )
                    route[0, layer_idx] = prefix_forced_route_decision(
                        layer_idx,
                        prefix_layers,
                        predicted_visual_on=predicted_visual_on,
                    )

            if capture_prefix_gate_features and layer_idx == prefix_layers:
                if binary_inputs.instruction_valid_mask is None:
                    raise ValueError("prefix gate feature capture requires instruction_token_mask")
                instruction = summarize_text_states(
                    text_states.detach(),
                    binary_inputs.instruction_valid_mask.to(text_states.device),
                )
                visual = summarize_visual_states(
                    visual_states.detach(),
                    binary_inputs.visual_valid_mask.to(visual_states.device),
                )
                prefix_gate_features = {
                    "instruction_mean": instruction["global_mean"].detach().float().cpu(),
                    "instruction_window_mean": instruction["window_mean"].detach().float().cpu(),
                    "instruction_last": instruction["last_token"].detach().float().cpu(),
                    "visual_summaries": visual.detach().float().cpu(),
                }
                if prefix_admission_callback is not None:
                    prefix_admission_used_sparse = bool(
                        prefix_admission_callback(prefix_gate_features)
                    )

            if route_mode == _ROUTER_ONLINE_MODE:
                route[0, layer_idx] = prefix_admitted_route_decision(
                    layer_idx,
                    prefix_layers,
                    use_sparse=prefix_admission_used_sparse,
                    predicted_visual_on=bool(route[0, layer_idx].item()),
                )
    ...
    return BinaryDVRCPrefillState(
        ...
        prefix_gate_features=prefix_gate_features,
        prefix_admission_used_sparse=prefix_admission_used_sparse,
    )
...
def _continue_binary_dvrc_greedy_generate(
    ...
    *,
    ...
    token_margins: list[torch.Tensor] = []
    ...
    for step in range(max_new_tokens):
        ...
        if return_token_scores:
            selected, entropy, margin = selected_token_distribution_stats(
                processed_logits, next_token
            )
            token_logprobs.append(selected.detach().cpu())
            token_entropies.append(entropy.detach().cpu())
            token_margins.append(margin.detach().cpu())
    ...
    return BinaryDVRCGenerationOutput(
        ...
        generated_token_logprobs=(
            torch.stack(token_logprobs, dim=1) if token_logprobs else None
        ),
        generated_token_entropies=(
            torch.stack(token_entropies, dim=1) if token_entropies else None
        ),
        generated_token_top1_top2_margins=(
            torch.stack(token_margins, dim=1) if token_margins else None
        ),
    )
...
@torch.inference_mode()
def binary_dvrc_forced_sequence_stats(
    model: DVRQwen2_5_VLForConditionalGeneration,
    inputs: Mapping[str, Any],
    visual_on_mask: torch.Tensor,
    ...
    *,
    ...
    """Teacher-force a candidate sequence under one static visual route."""

    tokens = torch.as_tensor(token_ids, dtype=torch.long)
    if tokens.ndim == 1:
        tokens = tokens.unsqueeze(0)
    if tokens.ndim != 2 or tokens.shape[0] != 1 or tokens.shape[1] == 0:
        raise ValueError("token_ids must have non-empty shape [1, steps]")
    state = binary_dvrc_prefill(
        model,
        inputs,
        visual_on_mask=visual_on_mask,
    ...
    logits = logits_from_binary_prefill(model, state)
    penalty = generation_repetition_penalty(model, repetition_penalty)
    forced_prefix: list[torch.Tensor] = []
    logprobs: list[torch.Tensor] = []
    entropies: list[torch.Tensor] = []
    margins: list[torch.Tensor] = []
    for step in range(tokens.shape[1]):
        token = tokens[:, step].to(logits.device)
        history = generation_token_history(
            inputs["input_ids"], forced_prefix, device=logits.device
        )
        processed = apply_repetition_penalty(logits, history, penalty)
        selected, entropy, margin = selected_token_distribution_stats(processed, token)
        logprobs.append(selected.detach().cpu())
        entropies.append(entropy.detach().cpu())
        margins.append(margin.detach().cpu())
        forced_prefix.append(token[:, None])
        if step + 1 < tokens.shape[1]:
            logits, _ = binary_dvrc_decode_one_step(
                model, token, state, generated_step=step
            )
    return {
        "token_logprobs": torch.stack(logprobs, dim=1),
        "token_entropies": torch.stack(entropies, dim=1),
        "token_top1_top2_margins": torch.stack(margins, dim=1),
    }
...
def binary_dvrc_router_greedy_generate(
    ...
    state = binary_dvrc_prefill(
        ...
        prepared_binary_inputs=prepared_binary_inputs,
        forced_visual_on_prefix_layers=forced_visual_on_prefix_layers,
        capture_prefix_gate_features=capture_prefix_gate_features,
        prefix_admission_callback=prefix_admission_callback,
    )
    return _continue_binary_dvrc_greedy_generate(
        model,
        inputs,
