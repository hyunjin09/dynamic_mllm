...
def one_replica_mode_topology(mode: str) -> dict[str, int]:
    if mode == "full":
        return dict(ONE_REPLICA_TOPOLOGY)
    return BASE_MODE_TOPOLOGY(mode)


def build_one_replica_execution_contract(
    *, launcher_path: Path | None = None, **kwargs: Any
) -> dict[str, Any]:
    """Extend the frozen scientific contract with truthful 8-worker provenance."""
    contract = deepcopy(BASE_BUILD_EXECUTION_CONTRACT(**kwargs))
    contract.pop("contract_sha256", None)
    contract["worker_topology"] = dict(ONE_REPLICA_TOPOLOGY)
...
def main() -> int:
    base_runner.mode_topology = one_replica_mode_topology
    base_runner.build_sequential_execution_contract = build_one_replica_execution_contract
