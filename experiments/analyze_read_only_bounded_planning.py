...
def aggregate():
    ...
    for uid,r in pop.items():
        ...
        for alg,seed in algorithms:
            ...
            if alg=='enumeration': budgets=list(BUDGETS)
                ...
                for b in (256,512):
                    ex=json.loads((ROOT/f'work/extension_{b}.json').read_text())
    ...
    cdf=df[~df.dense_wrong & df.budget.isin([8,32,128])].copy(); cdf['regression']=~cdf.selected_correct; cdf['dense_remains_selected']=[s=='1'*t for s,t in zip(cdf.selected_suffix,cdf.T)] if False else [s=='1'*t for s,t in zip(cdf['selected_suffix'],cdf['T'])]
    ...
    for b in [1,4,8,16,32,64,128]:
        pivot=u[u.budget==b].pivot(index='uid',columns='algorithm',values='any_correct').reindex(wids)
        for a,other in [(a,r) for a in ('greedy','beam2','beam4','beam8','enumeration') for r in ('random_uniform','random_sparse')]+[(a,'greedy') for a in ('beam2','beam4','beam8')]:
            delta=(pivot[a]-pivot[other]).to_numpy(); low,high=pair_boot(delta)
            tests.append(dict(algorithm=a,comparison=other,budget=b,uids=len(delta),mean_any_difference=float(delta.mean()),ci_low=low,ci_high=high,ceiling_recovery_difference=float(delta.mean()*1307/ceiling_n) if ceiling_n else None,matched='same_at_most_B_with_actual_counts_reported'))
    ...
    for _,cell in agg[agg.algorithm.isin(['greedy','beam2','beam4','beam8','enumeration'])].iterrows():
        ...
        a=cell.budget<=32 and cell.ceiling_recovery>=.70 and clear and delta10 and median<=32
        b=cell.budget<=128 and cell.ceiling_recovery>=.70 and clear
        candidates.append(dict(algorithm=cell.algorithm,budget=int(cell.budget),A=bool(a),B=bool(b),median_first_rescue=median))
    if any(c['A'] for c in candidates): category='P-READ-A'
    elif any(c['B'] for c in candidates): category='P-READ-B'
    else:
        top=agg[(agg.budget==128)&agg.algorithm.isin(['greedy','beam2','beam4','beam8','enumeration'])]
        ...
        strong=any(c['B'] for c in candidates)
        ...
        category='P-READ-C' if (low_recovery or high_gain) and (not strong or med<=.01) else 'INCONCLUSIVE'
    ...
    fig,ax=plt.subplots(figsize=(8,5))
    for alg,rr in firstdf[firstdf.dense_wrong].groupby('algorithm'):
...
9. Beam versus both random baselines and greedy: controls/random_search_comparison.csv and greedy_vs_beam.csv with5000 paired UID bootstrap draws (seeds averaged per UID).
...
Existence is supported: READ suppression can improve q and correct answers. Population adjacency is mostly isolated (R-STRUCT-B); nuisance-matched operation features provide no robust local mechanism (R-MECH-B). Generic, operation-specific, one-step and H<=8 predictions remain weak (R-LEARN-C, CaseD, H-READ-D). This study adds bounded gold-q trajectory search: {category}, {ceiling_status}, empirical correction coverage{ceiling_n}/1307. See the main summary and paired comparisons for the scope of trajectory-structure claims. Oracle plan discovery and q-selection are reported separately; neither establishes a deployable method.\n''')
    ...
    paths=sorted(set(p.resolve() for p in paths+external))
