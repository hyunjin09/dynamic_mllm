...
readonly config="configs/sequential_four_action_label_conversion_one_replica.yaml"
...
PYTHONPATH=. python experiments/monitor_route_conditioned_gpu.py \
    --output "analysis/4action_sequential_label_conversion_one_replica/vqa_first_${SLURM_JOB_ID}_gpu_telemetry.csv" \
    --interval-seconds 5 \
    --resume &
telemetry_pid=$!
...
torchrun --standalone --nproc_per_node=8 \
    experiments/run_sequential_four_action_label_conversion_one_replica.py \
    --config "${config}" \
    --mode full \
    --resume

# Continuation is submitted only if this topology beats the 16-worker baseline.
