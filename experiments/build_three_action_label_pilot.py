...
def main() -> int:
    parser = argparse.ArgumentParser(description="Freeze the answer-aligned three-action pilot.")
    parser.add_argument(
        "--source-manifest",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("datasets/mcts_labels_3action/conversion_v1/pilot"),
    )
    ...
    summary = {
        "schema_version": "three_action_label_conversion_pilot_summary_v1",
        ...
        "selection_rule": (
            "five-dataset quota; source W2C/C2C/ALL-OFF and multi-route proxies; "
            "short/long/high-cost deterministic coverage. Hard/soft paths are audited "
            "after current-runtime score screening and must be exercised before full launch"
        ),
    }
    write_once(
