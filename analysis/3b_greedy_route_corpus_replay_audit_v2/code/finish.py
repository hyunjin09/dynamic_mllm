...
from execution import read_execution_contract as read_contract
...
def pair_status(pair, current):
    ...
    elif ca!=old_a and cb==old_b:
        status='PREFERRED_CHANGED_ONLY'
    elif ca==old_a and cb!=old_b:
        status='REJECTED_CHANGED_ONLY'
    else:
        status='BOTH_LABELS_CHANGED'
...
def finish():
    ...
    semantic=Counter();by_length=Counter();by_composition=Counter()
    ...
    paths=['routes/current_route_replay.jsonl','routes/replay_errors.jsonl','filtering/current_route_manifest.jsonl','geometry_readiness/w_to_c_samples.jsonl','geometry_readiness/c_to_c_samples.jsonl']
    with ExitStack() as stack:
        ...
        for si,item in enumerate(index):
            ...
            for r in rs:
                emit('routes/current_route_replay.jsonl',r)
                current_counts[r['replay_status']]+=1
                if r['replay_status']=='ERROR':emit('routes/replay_errors.jsonl',r)
                cat=transition(dc,r['current_correctness'])
                derived=dict(r,old_dense_answer=dense['old_answer'],old_dense_correctness=dense['old_correctness'],
                    current_dense_answer=dense['current_answer'],current_dense_correctness=dc,
                    current_route_answer=r['current_answer'],current_route_correctness=r['current_correctness'],
                    old_route_answer=r['old_answer'],old_route_correctness=r['old_correctness'],current_route_category=cat)
                semantic[(r['dataset'],transition(dense['old_correctness'],r['old_correctness']),cat)]+=1
                by_length[(r['route_length'],r['label_transition'])]+=1
                by_composition[(LAYERS-r['interventions'],r['interventions'],r['label_transition'])]+=1
            ...
            dense_rows.append(dict(sample_uid=uid,dataset=sample['benchmark'],dense_route_key=dense['route_key'],current_dense_answer=dense['current_answer'],current_dense_correctness=dc,old_dense_answer=dense['old_answer'],old_dense_correctness=dense['old_correctness'],dense_label_transition=transition(dense['old_correctness'],dc),replay_status=dense['replay_status']))
    ...
    certified={r['sample_uid']:r for r in rows(OUT/'dense/current_dense_manifest.jsonl')}
    for row in dense_rows:
        assert all(row[k]==certified[row['sample_uid']][k] for k in ['current_dense_answer','current_dense_correctness','old_dense_answer','old_dense_correctness','dense_route_key'])
    ...
    write_csv(OUT/'transitions/route_transition_matrix.csv',[dict(transition=t,routes=n,percent=100*n/total) for t,n in sorted(transitions.items())])
    for filename,counts,axis in [('route_transition_by_dataset.csv',by_dataset,'dataset'),('route_transition_by_phase.csv',by_phase,'phase'),('route_transition_by_nonfull_count.csv',by_intervention,'interventions'),('route_transition_by_provenance.csv',by_family,'provenance'),('route_transition_by_length.csv',by_length,'route_length')]:
        totals=Counter()
        for (cell,tr),n in counts.items():totals[cell]+=n
        write_csv(OUT/'transitions'/filename,[{axis:cell,'transition':tr,'routes':n,'percent':100*n/totals[cell]} for (cell,tr),n in sorted(counts.items())])
    write_csv(OUT/'transitions/semantic_route_category_transition.csv',[dict(dataset=b,old_category=o,current_category=n,routes=count) for (b,o,n),count in sorted(semantic.items())])
    write_csv(OUT/'transitions/route_transition_by_action_composition.csv',[dict(full_layers=f,nonfull_layers=n,transition=t,routes=count) for (f,n,t),count in sorted(by_composition.items())])
    per_dataset=[]
    ...
    storage=dict(last_question_bytes=dict(BF16=current_text,FP16=current_text,FP32=current_text*2),last_question_bf16_bytes=current_text,visual_bytes=current_visual,visual_tokens_source='current successful replay token counts',free_bytes=shutil.disk_usage(OUT).free,hidden_capture='deferred')
    atomic_json(OUT/'storage/current_estimate.json',storage)
    write_text(OUT/'storage/last_question_token_storage_estimate.md',f"Current replay-valid routes × actual {LAYERS} layers × {c['hidden_size']} dimensions ×2 = {current_text:,} BF16 bytes ({current_text/2**30:.2f} GiB), excluding indexing. FP16 uses the same bytes; FP32 uses {current_text*2:,} bytes. No hidden tensors were collected.\n")
    ...
    dense_transitions=dict(Counter(r['dense_label_transition'] for r in dense_rows))
    summary=dict(dense_transitions=dense_transitions,current_route_categories=[dict(dataset=b,category=cat,routes=v['routes'],samples=len(v['samples'])) for (b,cat),v in sorted(categories.items())],samples=len(index),routes=total,replay=dict(current_counts),transitions=dict(transitions),
    ...
    write_text(OUT/'summaries/replay_filtering_summary.md','Complete current-server replay evidence.\n\n'+json.dumps(summary,indent=2)+'\n\nDense and routed transitions are separately reported under transitions/. Each route preserves current dense and route semantics. The source runtime differs; old labels were preserved. Nonzero drift requires qualitative RS-A/B/C review of actual rates and eligibility impact, without invented numerical thresholds.\n')
    census=read_json(OUT/'census/global_census.json')
    write_text(OUT/'summaries/corpus_audit_summary.md','Current-Server 3B Route Corpus Audit — V2\n\n'+json.dumps(dict(unique_samples=census['unique_samples'],unique_image_groups=census['unique_image_groups'],unique_routes=census['unique_routes'],routes_per_sample=census['routes_per_sample'],old_correct=census['saved_correct'],old_wrong=census['saved_wrong'],current=summary),indent=2)+'\n\nDataset-specific evidence: summaries/current_dataset_summary.csv.\n')
    ...
    write_text(OUT/'summaries/executor_parity_summary.md','Initial job2939 failed31/32 anchors. Diagnostic2941 located the first numerical divergence at layer0 prefill SDPA, with identical inputs/RoPE/QKV. The native implicit causal-mask/GQA convention exactly recovered the native SDPA output in an isolated check. The V2 adapter uses that convention only for unpadded FULL prefill. Padded and text-only masks remain unchanged.\n\nThe versioned repair gate and subsequent eight-worker preflights passed before full replay. Evidence: parity/repaired_gate_a_result.json, contracts/parity_repair_v1.json, replay_gate/job_*/. Canonical package unchanged.\n')
    ...
    for p in sorted(OUT.rglob('*')):
        if p.is_file() and p.suffix not in ['.pyc'] and not any(part in ['logs','cache','tmp','__pycache__'] for part in p.relative_to(OUT).parts) and p.name not in ['artifact_manifest.json','final_verification.json'] and '.tmp.' not in p.name:
